...
$a="select sp_name from sports where sno='".$eno."';";
$b=mysql_fetch_array(mysql_query($a));
//echo "<br>".$b[0];

$q="update sports set sp_name='".$name."',date='".$date."',time='".$time."' where sp_name='".$b[0]."';";
$z=mysql_query($q);
if(!$z)
echo mysql_error();
else
echo "<font color=white> Record updated";
$p=mysql_query("select * from sports");
if(!$p)
echo mysql_error();
else
echo "<table border=1 align=center bgcolor=black><tr><td><font color=white>Sports no</font></td><td><font color=white>Sports name</font</td><td><font color=white>Date</font></td><td><font color=white>Time</font></td></tr>";
while($c=mysql_fetch_array($p))
{
echo "<tr><td><font color=white>".$c[0]."</font><font color=white></td><td>".$c[1]."</font></td><td><font color=white>".$c[2]."</font></td><td><font color=white>".$c[3]."</font></td></tr>";
}
echo "</table></center>";
?>
