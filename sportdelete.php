...
echo "<html>";
//$id=$_GET['s'];
$q="delete from sports where sno='".$eno."'";
$z=mysql_query($q);
if(!$z)
echo mysql_error();
else
echo "<br><center>record deleted";
$p=mysql_query("select * from sports");
if(!$p)
echo mysql_error();
else
echo "<table border=1 bgcolor=white><tr><td>Sports no</td><td>Sports name</td><td>Date</td><td>Time</td></tr>";
while($c=mysql_fetch_array($p))
{
echo "<tr><td>".$c[0]."</td><td>".$c[1]."</td><td>".$c[2]."</td><td>".$c[3]."</td></tr>";

//<td><a href=edit.php>Edit</td><td><a href=delete.php>Delete</td></tr>";
}
echo "</table></center>";
